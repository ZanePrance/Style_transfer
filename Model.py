...
class AdaIN_net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        if self.training:  # training
            ...
            content_feats = content_features[-1]
            style_features = self.encode(style)

            t = self.adain(content_feats, style_features[-1])
            t = alpha * t + (1 - alpha) * content_feats

            g_t = self.decode(t)
            t_prime_feats = self.encode(g_t)

            loss_c = self.content_loss(t_prime_feats[-1], t)

            loss_s = self.style_loss(t_prime_feats[0], style_features[0])

            for i in range(1, 4):
                loss_s += self.style_loss(t_prime_feats[i], style_features[i])
...
# reference https://github.com/naoto0804/pytorch-AdaIN
